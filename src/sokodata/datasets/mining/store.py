"""SQLite warehouse for mining dataset."""
...
SCHEMA = """
CREATE TABLE IF NOT EXISTS mining_annual (
    date                  TEXT PRIMARY KEY,
    mineral_rents_pct_gdp REAL,
    ore_metal_exports_pct REAL,
    fuel_exports_pct      REAL,
...
def load_tables(conn: sqlite3.Connection, annual: pd.DataFrame) -> dict[str, int]:
    ...
    try:
        conn.execute("DELETE FROM mining_annual")
        if not annual.empty:
            annual.to_sql("mining_annual", conn, if_exists="append", index=False)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
