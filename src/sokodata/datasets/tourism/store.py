"""SQLite warehouse for tourism dataset."""
...
SCHEMA = """
CREATE TABLE IF NOT EXISTS tourism_annual (
    date                     TEXT PRIMARY KEY,
    tourist_arrivals         REAL,
    tourism_receipts_usd     REAL,
    tourism_expenditure_usd  REAL,
...
def load_tables(conn: sqlite3.Connection, annual: pd.DataFrame) -> dict[str, int]:
    ...
    try:
        conn.execute("DELETE FROM tourism_annual")
        if not annual.empty:
            annual.to_sql("tourism_annual", conn, if_exists="append", index=False)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
