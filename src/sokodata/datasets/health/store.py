"""SQLite warehouse for health dataset."""
...
SCHEMA = """
CREATE TABLE IF NOT EXISTS health_annual (
    date                TEXT PRIMARY KEY,
    infant_mort_per_1k  REAL,
    under5_mort_per_1k  REAL,
    measles_imm_pct     REAL,
    health_xpd_pct_gdp  REAL,
...
def load_tables(conn: sqlite3.Connection, annual: pd.DataFrame) -> dict[str, int]:
    ...
    try:
        conn.execute("DELETE FROM health_annual")
        if not annual.empty:
            annual.to_sql("health_annual", conn, if_exists="append", index=False)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
