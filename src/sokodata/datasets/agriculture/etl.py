"""Agriculture ETL: World Bank + FAO FAOSTAT + Agritex scrape -> SQLite."""
...
from sokodata.datasets.agriculture.clean import clean_annual, clean_fao
from sokodata.datasets.agriculture.fetch import fetch_agriculture_all
from sokodata.datasets.agriculture.store import connect, load_tables
...
def run_etl(data_dir: Path = RAW_DIR, db_path: Path = DB_PATH) -> dict[str, int]:
    ...
    try:
        counts = load_tables(conn, annual, fao)
    finally:
        conn.close()
    log.info("agriculture loaded %s into %s", counts, db_path)
    return counts
