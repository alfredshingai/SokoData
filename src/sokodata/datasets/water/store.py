"""SQLite warehouse for water dataset."""
...
SCHEMA = """
CREATE TABLE IF NOT EXISTS water_annual (
    date                 TEXT PRIMARY KEY,
    safe_water_pct       REAL,
    safe_sanitation_pct  REAL,
    basic_water_pct      REAL,
    basic_sanitation_pct REAL,
...
def load_tables(conn: sqlite3.Connection, annual: pd.DataFrame) -> dict[str, int]:
    ...
    try:
        conn.execute("DELETE FROM water_annual")
        if not annual.empty:
            annual.to_sql("water_annual", conn, if_exists="append", index=False)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
