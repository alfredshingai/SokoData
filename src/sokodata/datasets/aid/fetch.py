...
WDI_ODA = "https://api.worldbank.org/v2/country/ZW/indicator/DT.ODA.ALLD.CD?format=json&per_page=100&date=1960:2030"
WDI_ODA_GNI = "https://api.worldbank.org/v2/country/ZW/indicator/DT.ODA.ODAT.GN.ZS?format=json&per_page=100&date=1960:2030"
WDI_ODA_PC = "https://api.worldbank.org/v2/country/ZW/indicator/DT.ODA.ODAT.PC.ZS?format=json&per_page=100&date=1960:2030"

OECD_URL = "https://www.oecd.org/dac/financing-sustainable-development/development-finance-data/"
ODA_PDF = "https://www.oecd.org/dac/financing-sustainable-development/development-finance-data/ODA_Report.pdf"
...
def fetch_worldbank_aid() -> pd.DataFrame:
    oda = _fetch_wdi(WDI_ODA, "net_oda_usd")
    gni = _fetch_wdi(WDI_ODA_GNI, "oda_pct_gni")
    pc = _fetch_wdi(WDI_ODA_PC, "oda_per_capita_usd")
...
def fetch_oecd_html(raw_dir: Path | None = None) -> pd.DataFrame:
    tables = fetch_html_tables(OECD_URL, match="ODA|Zimbabwe|Aid|Assistance")
    rows = []
    for tbl in tables:
...
def fetch_aid_all(raw_dir: Path | None = None) -> dict[str, pd.DataFrame]:
    wdi = fetch_worldbank_aid()
    html = fetch_oecd_html(raw_dir)
    try:
        base = raw_dir or RAW_DIR
        pdf_path = Path(base) / "oda_report.pdf"
        download_pdf(ODA_PDF, pdf_path)
