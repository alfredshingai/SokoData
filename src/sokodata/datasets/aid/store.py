"""SQLite warehouse for aid dataset."""
...
SCHEMA = """
CREATE TABLE IF NOT EXISTS aid_annual (
    date               TEXT PRIMARY KEY,
    net_oda_usd        REAL,
    oda_pct_gni        REAL,
    oda_per_capita_usd REAL,
...
def load_tables(conn: sqlite3.Connection, annual: pd.DataFrame) -> dict[str, int]:
    ...
    try:
        conn.execute("DELETE FROM aid_annual")
        if not annual.empty:
            annual.to_sql("aid_annual", conn, if_exists="append", index=False)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
