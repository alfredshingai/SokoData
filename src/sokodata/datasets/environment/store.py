"""SQLite warehouse for environment dataset."""
...
SCHEMA = """
CREATE TABLE IF NOT EXISTS environment_annual (
    date             TEXT PRIMARY KEY,
    co2_per_capita_t REAL,
    forest_pct       REAL,
    pm25_ug_m3       REAL,
    forest_km2       REAL,
...
def load_tables(conn: sqlite3.Connection, annual: pd.DataFrame) -> dict[str, int]:
    ...
    try:
        conn.execute("DELETE FROM environment_annual")
        if not annual.empty:
            annual.to_sql("environment_annual", conn, if_exists="append", index=False)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
