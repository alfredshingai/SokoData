"""SQLite warehouse for transport dataset."""
...
SCHEMA = """
CREATE TABLE IF NOT EXISTS transport_annual (
    date              TEXT PRIMARY KEY,
    air_departures    REAL,
    rail_km           REAL,
    road_km           REAL,
    internet_use_pct  REAL,
...
def load_tables(conn: sqlite3.Connection, annual: pd.DataFrame) -> dict[str, int]:
    ...
    try:
        conn.execute("DELETE FROM transport_annual")
        if not annual.empty:
            annual.to_sql("transport_annual", conn, if_exists="append", index=False)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
