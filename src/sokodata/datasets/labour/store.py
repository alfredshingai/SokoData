"""SQLite warehouse for labour dataset."""
...
SCHEMA = """
CREATE TABLE IF NOT EXISTS labour_annual (
    date                              TEXT PRIMARY KEY,
    unemployment_pct                  REAL,
    unemployment_national_pct         REAL,
    labour_force_participation_pct    REAL,
    vulnerable_employment_pct         REAL,
...
def load_tables(conn: sqlite3.Connection, annual: pd.DataFrame) -> dict[str, int]:
    ...
    try:
        conn.execute("DELETE FROM labour_annual")
        if not annual.empty:
            annual.to_sql("labour_annual", conn, if_exists="append", index=False)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
