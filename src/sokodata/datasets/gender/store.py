"""SQLite warehouse for gender dataset."""
...
SCHEMA = """
CREATE TABLE IF NOT EXISTS gender_annual (
    date                         TEXT PRIMARY KEY,
    women_parliament_pct         REAL,
    female_lfpr_pct              REAL,
    primary_parity_index         REAL,
    maternal_mortality_per_100k  REAL,
...
def load_tables(conn: sqlite3.Connection, annual: pd.DataFrame) -> dict[str, int]:
    ...
    try:
        conn.execute("DELETE FROM gender_annual")
        if not annual.empty:
            annual.to_sql("gender_annual", conn, if_exists="append", index=False)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
