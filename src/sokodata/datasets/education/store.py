"""SQLite warehouse for education dataset."""
...
SCHEMA = """
CREATE TABLE IF NOT EXISTS education_annual (
    date                    TEXT PRIMARY KEY,
    primary_ner_pct         REAL,
    secondary_ner_pct       REAL,
    adult_literacy_pct      REAL,
    primary_completion_pct  REAL,
...
def load_tables(conn: sqlite3.Connection, annual: pd.DataFrame) -> dict[str, int]:
    ...
    try:
        conn.execute("DELETE FROM education_annual")
        if not annual.empty:
            annual.to_sql("education_annual", conn, if_exists="append", index=False)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
