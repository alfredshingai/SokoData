"""SQLite warehouse for poverty dataset."""
...
SCHEMA = """
CREATE TABLE IF NOT EXISTS poverty_annual (
    date                  TEXT PRIMARY KEY,
    extreme_poverty_pct   REAL,
    gini_index            REAL,
    national_poverty_pct  REAL,
    bottom40_growth_pct   REAL,
...
def load_tables(conn: sqlite3.Connection, annual: pd.DataFrame) -> dict[str, int]:
    ...
    try:
        conn.execute("DELETE FROM poverty_annual")
        if not annual.empty:
            annual.to_sql("poverty_annual", conn, if_exists="append", index=False)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
