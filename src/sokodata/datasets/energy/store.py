"""SQLite warehouse for energy dataset."""
...
CREATE TABLE IF NOT EXISTS energy_annual (
    ...
    elec_access_pct     REAL,
    elec_use_kwh_pc     REAL,
    energy_imports_pct  REAL,
    renewable_pct       REAL,
...
def load_tables(conn: sqlite3.Connection, annual: pd.DataFrame) -> dict[str, int]:
    ...
    try:
        conn.execute("DELETE FROM energy_annual")
        if not annual.empty:
            annual.to_sql("energy_annual", conn, if_exists="append", index=False)
        conn.commit()
    except Exception:
        conn.rollback()
        raise
